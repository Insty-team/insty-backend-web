...
public class CourseNotificationManager {
     ...
     * 새 질문 작성 알림 전송
     */
    public void sendNewQuestionNotification(CourseQuestion question) {
     ...
     * 답변 채택 알림 전송
     */
    public void sendAnswerAcceptedNotification(CourseQuestion question, CourseAnswer answer) {
        User creator = question.getCourse().getUser();
        User questionAuthor = question.getUser();
        Set<User> participants = courseAnswerReader.getParticipantsByQuestionId(question.getId());

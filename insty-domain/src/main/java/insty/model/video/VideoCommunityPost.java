...
@Slf4j
@Entity
@Table(name = "video_community_posts", schema = "web_service")
@Getter
@Builder(access = AccessLevel.PROTECTED)
@NoArgsConstructor(access = AccessLevel.PROTECTED)
@AllArgsConstructor(access = AccessLevel.PRIVATE)
public class VideoCommunityPost extends BaseEntity implements BaseVideo {
    ...
    public static VideoCommunityPost create(String fileName, UUID uuid, User user) {
        ...
        return VideoCommunityPost.builder()
                .videoUuid(uuid)
                .user(user)
                .s3Key(s3BucketKey)
                .extension(extension)
    ...
    private static String getS3BucketKey(String fileName, UUID uuid) {
        String extension = FileUtils.extractExtension(fileName)
                .orElseThrow(() -> new CustomException(VideoErrorCode.VIDEO_INVALID_FILE_NAME));
        return "vod/" + VideoType.COMMUNITY_POST + "/" + extension + "/" + uuid + "/" + fileName;
    }

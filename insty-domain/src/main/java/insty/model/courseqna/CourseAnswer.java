...
@Slf4j
@Entity
@Table(name = "course_answers", schema = "web_service")
@Getter
...
public class CourseAnswer extends BaseEntity {
    ...
    @OneToMany(mappedBy = "courseAnswer", cascade = {CascadeType.PERSIST, CascadeType.REMOVE}, orphanRemoval = true)
    @Builder.Default
    private List<CourseAnswerFile> attachments = new ArrayList<>();


    public static CourseAnswer create(CourseQuestion courseQuestion, User user, String content) {
        validateCreate(courseQuestion, user, content);
